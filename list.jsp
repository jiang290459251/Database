	...
	<table class="table" width="100%" layoutH="109">
		<thead>
			<tr>
				...
				<th width="5%">结算金额</th>
				<th width="5%">结算手续费</th>
				<th width="5%">结算打款金额</th>
				<th width="5%">平台手续费成本</th>
				<th width="5%">平台手续费利润</th>
				<th width="5%">银行名称</th>
				<th width="5%">开户名</th>
				<th width="5%">开户账户</th>
				<th width="5%">收款手机号</th>
		...
		<tbody>
			<c:forEach var="item" items="${pageBean.recordList}" varStatus="s">
				<tr>
			        ...
			        <td><fmt:formatNumber value="${item.settFee}" pattern="0.00"/> 
			        </td>
			        <td><fmt:formatNumber value="${item.remitAmount}" pattern="0.00"/> 
			        </td>
			        <td><fmt:formatNumber value="${item.settCost}" pattern="0.00"/> 
			        </td>
			        <td><fmt:formatNumber value="${item.settProfit}" pattern="0.00"/> 
			        </td>
					<td>${item.bankName}</td>
